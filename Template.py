...
def data_block_loader_train_val_test_image_separate(use_all=False, eye_remove=True, do_norm=True, do_zscore=True):
    ...
    if use_all:
        ...
        file_name2 = 'Data/02_All/Block_Sub001_all.mat'
        full_data2 = loadmat(file_name2)
        file_name3 = 'Data/03_All/Block_Sub001_all.mat'
        full_data3 = loadmat(file_name3)
        file_name4 = 'Data/04_All/Block_Sub001_all.mat'
        full_data4 = loadmat(file_name4)
        EEGs = np.concatenate([full_data1['data'], full_data2['data'],
                               full_data3['data'], full_data4['data']], axis=0)
        full_data1_images=np.array([image.split('/')[1] for image in full_data1['images']])
        full_data2_images=np.array([image.split('/')[1] for image in full_data2['images']])
        full_data3_images=np.array([image.split('/')[1] for image in full_data3['images']])
        full_data4_images=np.array([image.split('/')[1] for image in full_data4['images']])
        images = np.concatenate([full_data1_images, full_data2_images,
                                 full_data3_images, full_data4_images], axis=0)
        labels = np.concatenate([full_data1['labels'], full_data2['labels'],
                                 full_data3['labels'], full_data4['labels']], axis=0)

    else:
        if eye_remove:
            file_name1 = 'data/Block_Sub001_Remove_Eye.mat'
            full_data1 =  loadmat(file_name1)
            file_name2 = 'data/Block_Sub002_Remove_Eye.mat'
            full_data2 = loadmat(file_name2)
            file_name3 = 'data/Block_Sub003_Remove_Eye.mat'
            full_data3 = loadmat(file_name3)
            file_name4 = 'data/Block_Sub004_Remove_Eye.mat'
            full_data4 = loadmat(file_name4)
            EEGs = np.concatenate([full_data1['data'], full_data2['data'],
                                   full_data3['data'], full_data4['data']], axis=0)
            data1_images=np.array([image.split('/')[1] for image in full_data1['images']])
            data2_images=np.array([image.split('/')[1] for image in full_data2['images']])
            data3_images=np.array([image.split('/')[1] for image in full_data3['images']])
            data4_images=np.array([image.split('/')[1] for image in full_data4['images']])
            images = np.concatenate([data1_images, data2_images,
                                     data3_images, data4_images], axis=0)
            labels = np.concatenate([full_data1['labels'], full_data2['labels'],
                                     full_data3['labels'], full_data4['labels']], axis=0)
    ...
    for i in range(images.shape[0]):
        image_path = 'finalEEGImages/' + images[i]
...
""" ResNet_Images = tf.keras.applications.resnet.preprocess_input(images_for_vis_feature)
    ...
    # Preprocess images for ShuffleNet
    ShuffleNet_Images = tf.keras.applications.shufflenet.preprocess_input(images_for_vis_feature)

    # Load ShuffleNet model and get features
    ShuffleNet = ShuffleNetV2()
    ShuffleNet_model = Model(ShuffleNet.input, ShuffleNet.layers[-2].output)  # Input = (224, 224, 3), Output = (1024,)
    ShuffleNet_Features = ShuffleNet_model.predict(ShuffleNet_Images)
    ...
    for i in range(40):
        ...
        for ii in range(5):
            ...
            img_path = part_list[ii][2]
            val_list.append(img_path)
            #img_path2 = 'finalEEGImages/' + part_list[ii + 5][2]
            img_path2 = part_list[ii + 5][2]
    ...
    for i in range(EEGs.shape[0]):
        if images[i] in val_list:
            ...
            validation_vis_features.append(ShuffleNet_Features[i])
            ...
            test_vis_features.append(ShuffleNet_Features[i])
            ...
            train_vis_features.append(ShuffleNet_Features[i])
...
def data_block_loader_train_test_image_separate2(use_all=False, eye_remove=True, do_norm=True, do_zscore=True):
    ...
    if use_all:
        ...
        EEGs = np.concatenate([full_data1['data'], full_data2['data'],
                               full_data3['data'], full_data4['data']], axis=0)
        full_data1['images']=np.array([image.split('/')[1] for image in full_data1['images']])	
        full_data2['images']=np.array([image.split('/')[1] for image in full_data2['images']])
        full_data3['images']=np.array([image.split('/')[1] for image in full_data3['images']])
        full_data4['images']=np.array([image.split('/')[1] for image in full_data4['images']])
        images = np.concatenate([full_data1['images'], full_data2['images'],
                                 full_data3['images'], full_data4['images']], axis=0)
        labels = np.concatenate([full_data1['labels'], full_data2['labels'],
                                 full_data3['labels'], full_data4['labels']], axis=0)
...
def data_imagination40_loader_train_val_test_sub001(do_norm=True, do_zscore=True):
    # Load data
    file_name1 = 'data/Imagine_Sub001.mat'
    full_data1 = loadmat(file_name1)
...
def data_imagination40_loader_train_val_test_sub002(do_norm=True, do_zscore=True):
    # Load data
    file_name1 = 'data/Imagine_Sub002.mat'
    full_data1 = loadmat(file_name1)
    EEGs = full_data1['data']
    full_data1['images']=np.array([image.split('/')[1] for image in full_data1['images']])
    images = full_data1['images']
...
def data_imagination40_loader_train_val_test_sub003(do_norm=True, do_zscore=True):
    # Load data
    file_name1 = 'data/Imagine_Sub003.mat'
    full_data1 = loadmat(file_name1)
    EEGs = full_data1['data']
    full_data1['images']=np.array([image.split('/')[1] for image in full_data1['images']])
    images = full_data1['images']
...
def data_imagination40_loader_train_val_test_sub004(do_norm=True, do_zscore=True):
    # Load data
    file_name1 = 'data/Imagine_Sub004.mat'
    full_data1 = loadmat(file_name1)
    EEGs = full_data1['data']
    full_data1['images']=np.array([image.split('/')[1] for image in full_data1['images']])
    images = full_data1['images']
...
def data_imagination40_loader_train_val_test_sub_all(do_norm=True, do_zscore=True):
    ...
    # Load data Sub002
    file_name2 = 'data/Imagine_Sub002.mat'
    full_data2 = loadmat(file_name2)
    EEGs = full_data2['data']
    full_data2['images']=np.array([image.split('/')[1] for image in full_data2['images']])
    images = full_data2['images']
    labels = full_data2['labels']
    ...
    # Load data Sub003
    file_name3 = 'data/Imagine_Sub003.mat'
    full_data3 = loadmat(file_name3)
    EEGs = full_data3['data']
    full_data3['images']=np.array([image.split('/')[1] for image in full_data3['images']])
    images = full_data3['images']
    labels = full_data3['labels']
    ...
    # Load data Sub004
    file_name4 = 'data/Imagine_Sub004.mat'
    full_data4 = loadmat(file_name4)
    EEGs = full_data4['data']
    full_data4['images']=np.array([image.split('/')[1] for image in full_data4['images']])
    images = full_data4['images']
    labels = full_data4['labels']
